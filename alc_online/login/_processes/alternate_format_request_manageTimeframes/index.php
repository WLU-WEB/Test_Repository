...
<table width="100%"  border="0" cellspacing="0" cellpadding="1">
	    <tr>
	      ...
	      <td width="99%" align="left" valign="top"><table width="100%"  border="0" cellspacing="3" cellpadding="0">
	        <tr>
	          <td width="99%"><h1><?php echo "Alternate Format Request: Manage Timeframes"; ?></h1></td>
            </tr>
	        <tr>
	    ...
	    <tr>
	      ...
	      <td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="7">
	        <tr align="left" valign="top">
	          <td align="right"><a href="<?PHP $PHP_SELF?>?action=alternateFormatRequestTimeFrames&amp;subAction=addNewTimeframes">Add New Timeframe</a> | <a href="<?PHP $PHP_SELF?>?action=alternateFormatRequestTimeFrames&amp;subAction=viewExistingTimeframes">View  Timeframes</a></td>
            </tr>
	        <tr align="left" valign="top">
	          <td><?php 
				  ...
				  if($action=="alternateFormatRequestTimeFrames" && !$subAction) 
					  { 
				  ...
				  if($submit_step=="Insert Timeframe") 
					  ...
					  if(count($errors)>0)
						  ...
						  $date_added_fF = date("Y-m-d H:i:m");
					  ...
					  $sql_process = "INSERT INTO a_alternate_request_timeframes (id,timeframe_unique_id,timeframe_name,timeframe_start,timeframe_end,added_by_user_name,date_added)
									  VALUES (NULL,'$timeframe_id_generated','$timeframe_name_fF','$timeframe_start_fF','$timeframe_end_fF','$ALC_username','$date_added_fF')";
						  $result_createNewProject = mysql_query($sql_process) or die ("Error in query: $sql_process. " .mysql_error());
						  mysql_close($dbcnx);	
							  
						  echo "<script language=javascript>setTimeout(\"location.href='index.php?action=alternateFormatRequestTimeFrames&subAction=addNewTimeframes'\",5000);</script>";
						  echo "Success! <A HREF='$PHP_SELF?action=alternateFormatRequestTimeFrames&subAction=addNewTimeframes'>Click here to add another timeframe.</A><P>";
						  echo "Redirecting in 5 seconds...<P>";
						  }
					  }
				  ...
				  if($submit_step=="Delete Timeframe") 
					  ...
					  if(count($errors)>0)
						  {
						  echo "<script language=javascript>setTimeout(\"location.href='index.php?action=manageTimeframes&subAction=viewExistingTimeframes'\",5000);</script>";
						  echo "Ensure you check the corresponding checkbox before deletion.<P>"; 
						  echo "<A HREF='$PHP_SELF?action=manageTimeframes&subAction=addNewTimeframes&subAction=viewExistingTimeframes'>Click here to return back.</A><P>";
						  echo "Redirecting in 5 seconds...<P>";
						  //return;
						  }
					  else
						...
						$dbcnx = @mysqlLogin($connection_name);	
						  ...
						  echo "<A HREF='$PHP_SELF?action=manageTimeframes&subAction=addNewTimeframes&subAction=viewExistingTimeframes'>Click here to delete another timeframe.</A><P>";
						  echo "Redirecting in 5 seconds...<P>";
						  }
					  }
