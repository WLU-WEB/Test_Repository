...
$link_to_redirect_after_submit = "/alconline/index.php?action=midtermExams";
...
if (!$submit) 
	...
	if(!$timeframe_unique_id_fdB)
		...
		$result_3 = mysql_query("SELECT * FROM " . $table . " WHERE timeframe_start >'$today_date' AND application_type='Midterm_Registration'");
...
if($submit=="Continue") 
	...
	if(count($errors)>0)
			...
			if($user_id_check_fdB) //request ID already exists in dB, do not process (ie. student has reloaded page)
				...
				$sql = "INSERT INTO exam_registrations (entry_id,date,user_id,registration_id,timeframe_unique_id,phone,instructor,course,section,term,date_required,start_time,exam_length,class_location,approved_accomodations,ip,host,campus)
					   VALUES (NULL,'$date_fF','$user_id_fdB','$generated_registration_id','$timeframe_unique_id_fdB','$phone_fF','$instructor_fF','$course_fF','$section_fF','$term_fF','$date_required_concatenated','$exam_time_concatenated','$exam_length_fF','$class_location_fF','$approved_accomodations_fF','$ip','$host','$campus_fF')";	
				...
				$subject_student = "Laurier ALC: Midterm Exam Registration Confirmation";
				...
				Your information for Midterm Exam has been successfully received.<P>";
				$message_student .= "
				...
				$headers_office = "From: Automated Midterm Exam Online Notice <alexams@wlu.ca>\r\n";
				$headers_office .= "Content-type: text/html; charset=iso-8859-1\r\n"; 
				$subject_office = "ALC: Midterm Exam Registration - $timeframe_name_fdB ($timeframe_start_fdB_verbose - $timeframe_end_fdB_verbose)";
				$message_office = "
				New Midterm Exam Registration submitted!<P>
				<B>Registration #:</B> $entry_id_fdB<P>
				<B>Date Processed:</B> $date_verbose_fF<P>
				<B>Name:</B> $first_name_fdB $last_name_fdB<BR>
				<B>MyLaurier User:</B> $user_novell_id_fdB<BR>
