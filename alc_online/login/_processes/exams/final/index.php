...
$result_form_type = mysql_query("SELECT * FROM " . $table . " WHERE timeframe_start <='$today_date' AND timeframe_end >='$today_date' AND application_type='Final_Registration'");
...
$link_to_redirect_after_submit = "/alconline/index.php?action=finalExams";
...
if (!$submit) 
	...
	if(!$timeframe_unique_id_fdB)
		...
		$result_3 = mysql_query("SELECT * FROM " . $table . " WHERE timeframe_start >'$today_date' AND application_type='Final_Registration'");
...
if($submit=="Continue") 
	...
	if(count($errors)>0)
			...
			if($user_id_check_fdB) //request ID already exists in dB, do not process (ie. student has reloaded page)
				...
				$sql = "INSERT INTO exam_registrations (entry_id,date,user_id,registration_id,timeframe_unique_id,phone,instructor,course,section,term,deferred,approved_accomodations,ip,host,campus)
					   VALUES (NULL,'$date_fF','$user_id_fdB','$generated_registration_id','$timeframe_unique_id_fdB','$phone_fF','$instructor_fF','$course_fF','$section_fF','$term_fF','$deferred_fF','$approved_accomodations_fF','$ip','$host','$campus_fF')";	
				...
				$subject_student = "Laurier ALC: Final Exam Registration Confirmation";
				...
				Your information for Final Exam has been successfully received.<P>";
				$message_student .= "
				...
				<B>Term</B> $term_fF<BR>
				<B>Deferred Exam</B> $deferred_fF<BR>";
				$message_student .= "
				...
				elseif($_SESSION['campus_fF']=='brantford'){
					$to_office = "lbalexams@wlu.ca";
				}
				else { $to_office = "alexams@wlu.ca"; }
				$headers_office = "From: Automated Final Exam Online Notice <alexams@wlu.ca>\r\n";
				$headers_office .= "Content-type: text/html; charset=iso-8859-1\r\n"; 
				$subject_office = "ALC: Final Exam Registration - $timeframe_name_fdB ($timeframe_start_fdB_verbose - $timeframe_end_fdB_verbose)";
				$message_office = "
				New Final Exam Registration submitted!<P>
				<B>Registration #:</B> $entry_id_fdB<P>
				<B>Date Processed:</B> $date_verbose_fF<P>
				<B>Name:</B> $first_name_fdB $last_name_fdB<BR>
				<B>MyLaurier User:</B> $user_novell_id_fdB<BR>
