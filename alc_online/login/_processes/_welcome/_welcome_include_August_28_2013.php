<?PHP
	...
	//Exam Registrations
	$result_exams = mysql_query("SELECT * FROM timeframes WHERE timeframe_start <='$today_date' AND timeframe_end >='$today_date'");
	$retrieved_value_exams = mysql_fetch_array($result_exams);
	...
	$timeframe_start_exams_fdB_verbose = date('l, F j, Y', strtotime($retrieved_value_exams['timeframe_start']));
	$timeframe_end_exams_fdB = $retrieved_value_exams['timeframe_end'];
	$timeframe_end_exams_fdB_verbose = date('l, F j, Y', strtotime($retrieved_value_exams['timeframe_end']));
	if(!$timeframe_unique_id_exams_fdB)
		{
		$result_exams_2 = mysql_query("SELECT * FROM timeframes WHERE timeframe_start >'$today_date'");
		...
		if($application_type_fdB=="Midterm_Registration")
			...
			if(!$timeframe_unique_id_exams_fdB)
				...
				if($timeframe_unique_id_next_exams_fdB)
					{
					echo "<BR>The next <B>Midterm</B> registration starts on <B>$timeframe_start_next_exams_fdB_verbose</B> and will end on <B>$timeframe_end_next_exams_fdB_verbose</B> at 11:59 pm.";
					}
				if(!$timeframe_unique_id_next_exams_fdB)
					{
					echo "<BR>The administrator has not yet specified when the next <B>Midterm</B> registration will start.";
					}
				}
			}
		?>
