<?PHP
//Exam Booking
if($action=="ExpressRegistrationTimeFrames")
	{ $title = "Express Registration for Returning Students"; }
	    ...
	    <tr>
	      ...
	      <td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="7">
	        <tr align="left" valign="top">
	          <td align="right"><a href="<?PHP $PHP_SELF?>?action=ExpressRegistrationTimeFrames&subAction=addNewTimeframes">Add New Timeframe</a> | <a href="<?PHP $PHP_SELF?>?action=ExpressRegistrationTimeFrames&subAction=viewExistingTimeframes">View  Timeframes</a></td>
            </tr>
	        <tr align="left" valign="top">
	          <td><?php 
				  ...
				  if($action=="ExpressRegistrationTimeFrames" && !$subAction) 
					  { 
				  ...
				  if($submit_step=="Insert Timeframe") 
					  ...
					  if(count($errors)>0)
						  ...
						  $timeframe_id_generated_array = array($year_start,$month_start,$day_start,$year_end,$month_end,$day_end);
					  ...
					  $sql_process = "INSERT INTO returning_students_timeframe (timeframe_unique_id,timeframe_name,timeframe_start,timeframe_end,added_by,date_added)
									  VALUES ('$timeframe_id_generated','$timeframe_name_fF','$timeframe_start_fF','$timeframe_end_fF','$ALC_username','$date_added_fF')";
									  
						  $result_createNewProject = mysql_query($sql_process) or die ("Error in query: $sql_process. " .mysql_error());
						 // mysql_close($dbcnx);	
							  
						  echo "<script language=javascript>setTimeout(\"location.href='index.php?action=ExpressRegistrationTimeFrames'\",5000);</script>";
						  echo "<h3>Timeframe Added Successfully</h3> <P>";
					  ...
					  if(count($errors)>0)
						...
						//$dbcnx = @mysqlLogin($connection_name);	
						 ...
						 // mysql_close($dbcnx);	
						  ...
						  echo "<script language=javascript>setTimeout(\"location.href='index.php?action=ExpressRegistrationTimeFrames&subAction=viewExistingTimeframes'\",3000);</script>";
						  echo "Successfully deleted timeframe.<P>"; 
						  echo "<A HREF='$PHP_SELF?action=ExpressRegistrationTimeFrames&subAction=addNewTimeframes&subAction=viewExistingTimeframes'>Click here to delete another timeframe.</A><P>";
						  echo "Redirecting in 3 seconds...<P>";
						  }
					  }
